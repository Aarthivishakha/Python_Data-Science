...
report. The implementation is compatible with Python 3.16 and uses only the
...
Native CI uses the supported `3.16-dev` selector. No official Python 3.16
container exists, so the multi-stage Dockerfile builds current CPython
development source. Ruff uses its newest available `py315` grammar target.
The isolated analyzer job remains on Python 3.10+ for reproducible execution.
...
- `pyproject.toml`: Python 3.16 package metadata, test, coverage, and Ruff configuration.
...
- `.github/workflows/ci.yml`: native Python 3.16 development tests plus isolated tool analysis.
