...
report. The implementation is compatible with Python 3.15 and uses only the
...
Python 3.15 is pre-release. Native CI pins `3.15.0-beta.4`, while Docker uses
the maintained official rolling `python:3.15-rc-slim` pre-release image. The
isolated analyzer job remains on Python 3.10+ for reproducible tool execution.
...
- `pyproject.toml`: Python 3.15 package metadata, test, coverage, and Ruff configuration.
...
- `.github/workflows/ci.yml`: native Python 3.15 beta tests plus isolated tool analysis.
